...
//
// Steps:
// 1. If the input `node` is `nullptr`, return `nullptr` because there's nothing to clone.
// 2. Create an `unordered_map` to map each original node to its cloned node.
...
public:
    Node* cloneGraph(Node* node) {
        ...
        auto dfs = [&](Node* node_1, auto& dfs_ref) -> Node* {
            if (map.count(node_1)) return map[node_1];

            Node* new_node = new Node(node_1->val);
            map[node_1] = new_node;
            for (Node* n: node_1->neighbors) {
                new_node->neighbors.push_back(dfs_ref(n, dfs_ref));
            }
